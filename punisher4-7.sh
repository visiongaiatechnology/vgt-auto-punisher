#!/bin/bash
# ==============================================================================
# VISIONGAIA TECHNOLOGY: AUTO-PUNISHER (V4.7.0 - SUPREME UI EDITION)
...
# UPDATE: Höchstes SSH-Terminal Design, Target-Port Visualisierung & Unicode-Tables.
# ==============================================================================
...
function init_defense() {
    ...
    echo -e "${C_CYAN}   VGT APEX HYBRID ELITE INITIALISIERUNG (V4.7.0)         ${C_RESET}"
    echo -e "${C_PURPLE}==========================================================${C_RESET}"
...
# ==============================================================================
# 2. DIE JAGD-LOGIK (V4.7.0 SUPREME UI & PORT DISCOVERY)
# ==============================================================================
function cleanup_ui() {
    echo -ne "${TUI_RMCUP}"
    tput cnorm
...
function start_hunt() {
    ...
    journalctl -kf --grep="$LOG_PREFIX" | awk -v ip_limit="$IP_THRESHOLD" -v r_limit="$RANGE_THRESHOLD" -v wr_limit="$WIDE_RANGE_THRESHOLD" -v v_limit="$VELOCITY_LIMIT" -v set_v4="$IPSET_V4" -v set_v6="$IPSET_V6" -v wl="$WHITELIST_IPS" '
        BEGIN {
            ...
            printf "%s│%s%s%-102s%s%s│%s\n", c_cyn, c_pur, c_bld, "   VGT AUTO-PUNISHER V4.7.0 - SUPREME TERMINAL DASHBOARD (L4 KINETICS)", c_res, c_cyn, c_res;
        ...
        /SRC=/ {
            ...
            if (ip_burst >= v_limit) { status_msg = "FLASH"; status_col = c_red; }
